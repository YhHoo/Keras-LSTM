...
def experiment(repeats, series, seed):
    ...
    for r in range(repeats):
        # fit the model
        batch_size = 4
        train_trimmed = train_scaled[2:, :]  # ignore row 0-1
        lstm_model = fit_lstm(train_trimmed, batch_size, 3000, 4)
        # forecast the entire training dataset to build up state for forecasting
        # as we can see the output from predict is ignored, simply want the train
        # set to go in to build up state
        if seed:
            train_reshaped = train_trimmed[:, 0].reshape(len(train_trimmed), 1, 1)
            lstm_model.predict(train_reshaped, batch_size=batch_size)
        # forecast test dataset
        # if seed is True, the model by now is carrying the state after predict(train)
...
# load dataset
series = read_csv('shampoo_sales_dataset.csv',
                  header=0,
                  parse_dates=[0],
                  index_col=0,
                  squeeze=True,
